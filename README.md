...
这个仓库保留核心工程代码、运行配置和项目文档，排除了本地依赖、私有环境文件和临时生成产物。
...
建议先使用确定性 Demo 模式启动。它不依赖外部 LLM 是否稳定，但仍会完整走后端、SSE 流、会话持久化、行程解析和前端渲染链路。
...
`DEMO_MODE=true` 用于稳定演示和本地调试。当外部 LLM、地图、天气或 POI 服务不可用时，它可以保持用户可见流程稳定。后端仍会执行结构化需求校验、SSE 输出、Redis 会话存储，并返回完整的行程结构供前端正常渲染。
